...
class Computer {
  constructor(public opponent: number[][] = [], public randomCoords: number[][] = [], public calculatedCoords: number[][] = []) {
  ...
  setCoordsForNextMove(x: number, y: number) {
    ...
    for (let r = 0; r < e.length; r += 1) {
      const i = e[r],
        o = i[0],
        n = i[1];
      o >= 0 && o <= 9 && n >= 0 && n <= 9 && this.opponent[o][n] !== 2 && this.opponent[o][n] !== 3 && this.calculatedCoords.push([o, n])
  ...
  removeCoordsFromArrays(c: number[]) {
    this.calculatedCoords.length > 0 && (this.calculatedCoords = Computer.removeFromArray(this.calculatedCoords, c)), this.randomCoords = Computer.removeFromArray(this.randomCoords, c)
  }
};
